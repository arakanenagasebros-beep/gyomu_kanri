...
let atlShouldReset = false;
let monthCheckShouldReset = false;
...
if(h==="#admin-task-list"){if(!data.session.adminAuthed){location.hash="#admin-login";return}atlShouldReset=true;showOnly("adminTaskList");renderAdminTaskList();return}
...
if(h==="#admin-month-check"){if(!data.session.adminAuthed){location.hash="#admin-login";return}monthCheckShouldReset=true;showOnly("adminMonthCheck");renderMonthCheck();return}
...
/* === ADMIN REPORT MGMT === */
let armInit=false;
function renderAdminReportMgmt(){renderAdminNotifications();const now=new Date();$("armMonthInfo").textContent=monthLabelJa(now);const dr=getAllUsersDateRange();buildYearMonthOpts($("armFilterYear"),$("armFilterMonth"),dr,!armInit);if(!armInit||armShouldReset){$("armFilterYear").value=String(now.getFullYear());$("armFilterMonth").value=String(now.getMonth()+1);$("armFilterType").value="在宅";$("armFilterUserType").value="全て";armInit=true;armShouldReset=false}doRenderARM()}
    ...
    if(isAdmin){
      tr.querySelectorAll("td").forEach((td,ci)=>{
        ...
        td.addEventListener("dblclick",()=>{
          ...
          else if(f==="staff"){inp=document.createElement("select");const _uo=document.createElement("option");_uo.value="未指定";_uo.textContent="未指定";inp.appendChild(_uo);getStaffNames().forEach(v=>{const o=document.createElement("option");o.value=v;o.textContent=v;inp.appendChild(o)});inp.value=getTaskStaffLabel(t)||"未指定";}
          ...
          const save=()=>{td.classList.remove("editing");
            ...
            else if(f==="staff"){setTaskStaffRef(t,inp.value);}
...
populateStaffSelect($("taStaff"),getTaskStaffLabel(t)||"未指定");
applyTaskTypeLogic();
...
function getAdminTaskSubTabs(){
  const tabs=[{ key:"全体", label:"全体", ref:null }];
  ["小笠原","諸富"].forEach(keyword=>{
    const user=getStaffUsers().find(u=>getUserDisplayName(u).includes(keyword));
    if(!user)return;
    tabs.push({ key:String(user.id||keyword), label:`${getUserDisplayName(user)}さん業務`, ref:String(user.id||getUserDisplayName(user)) });
  });
  return tabs;
}
function renderAdminTaskList(){
  ...
  if(atlShouldReset){
    const now=new Date();
    atlSubTab="全体";
    $("atlDateType").value="requestDate";
    $("atlYear").value=String(now.getFullYear());
    $("atlMonth").value=String(now.getMonth()+1);
    $("atlEmployee").value="全て";
    $("atlStaff").value="全て";
    $("atlStatus").value="全て";
    atlShouldReset=false;
  } else if(!$("atlStatus").value){
    $("atlStatus").value="全て";
  }
  // Sub tabs
  const subTabs=$("atlSubTabs");subTabs.innerHTML="";
  const tabs=getAdminTaskSubTabs();
  if(!tabs.some(t=>t.key===atlSubTab)) atlSubTab="全体";
  tabs.forEach(tab=>{const btn=document.createElement("div");btn.className="sub-tab"+(atlSubTab===tab.key?" active":"");btn.textContent=tab.label;
    btn.addEventListener("click",()=>{atlSubTab=tab.key;renderAdminTaskList()});subTabs.appendChild(btn)});
  doRenderATL();
}
function doRenderATL(){
  const tabs=getAdminTaskSubTabs();
  const currentTab=tabs.find(tab=>tab.key===atlSubTab)||tabs[0];
  const subStaff=currentTab&&currentTab.ref?currentTab.ref:null;
  const hideStaffs=(!subStaff&&atlSubTab==="全体")?tabs.filter(tab=>tab.ref).map(tab=>tab.ref):null;
...
renderAdminHome = function() {
  ...
  decorateAdminCardHeaderByBodyId("adminTbody", "🌟 ユーザー一覧", "スタンプ状況・申請状況・編集を確認できます");
...
renderMonthCheck = function() {
  ...
  decorateAdminCardHeaderByBodyId("mcTbody", "📋 月次確認", "対象月の在宅業務と在宅日報の工数を照合します");
};
...
renderMonthCheck = function() {
  ...
  if (monthCheckShouldReset) {
    $("mcYear").value = String(now.getFullYear());
    $("mcMonth").value = String(now.getMonth() + 1);
    monthCheckViewState.tab = "summary";
    monthCheckViewState.attendanceStaff = "全て";
    monthCheckViewState.taskStaff = "全て";
    monthCheckViewState.taskStatus = "全て";
    monthCheckViewState.taskMatchStatus = "全て";
    monthCheckViewState.guideOpen = false;
    monthCheckShouldReset = false;
  }
